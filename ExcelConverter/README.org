...
* Build, Run
  ...
  Should generate an executable jar with all dependencies, so you can deploy and run it simply with
  ~java -jar <jarFileName>~ (no zipping/unzipping required).

  Note that this doesn't (yet) handle logging configuration, so you'll have to extract the logging
  properties manually from the jar file to use it:

  : jar xvf <jarFileName> logging.properties
  : java -Djava.util.logging.config.file=logging.properties -jar <jarFileName> [options]

  (Note also that jar files are really just zip archives, so you can use any unzipping tool to
  extract the logging.properties file, if you happen not to have the ~jar~ command.)
